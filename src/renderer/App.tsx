...
function App() {
  ...
  useEffect(() => {
    if (settings && (settings.autoResizeVertical || settings.autoResizeHorizontal)) {
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetId: string) => {
    ...
    if (draggedId !== targetId) {
      ...
      if (draggedIndex !== -1 && targetIndex !== -1) {
        debugLog('Reordering tabs - from index', draggedIndex, 'to index', targetIndex);
  ...
  debugLog('Rendering app with', orderedWindows.length, 'windows, active:', activeWindowId);
